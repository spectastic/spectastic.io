---
name: spectastic-principles
description: Author or amend the project principles — the non-negotiable,
  project-wide rules every downstream spec must honor. Use when setting up a
  project's constitution, adding or changing a cross-cutting invariant, or
  deciding what rules every spec must obey — not a single feature's requirements
  (that's /spectastic.spec).
metadata:
  triggers:
  ...
  use-when: Establishing or amending the project-wide invariants every downstream
    spec must honor — the constitution, authored once and changed rarely.
  sibling-boundary: Not spec (a single feature's requirements) — principles are
    cross-cutting rules above any one feature.
---
...
User input (from the user's request): either the project name to ratify a new principles document, or a short description of an amendment to apply.
...
2. **Interview** the user (only if needed — skip questions whose answers are already in the user's request or in an existing principles document). Capture:
